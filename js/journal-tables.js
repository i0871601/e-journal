    ...
    document.querySelectorAll('thead th').forEach(headerCell => {
        if (headerCell.dataset.lessonNumber) {
            headerCell.addEventListener('click', () => {
                ...
                const checkedInfo = document.getElementById('checked-infoLesson');
                const container_dataText = document.querySelector("#infoData .text-info");
                const container_topicText = document.querySelector("#infoTopic .text-info");

                if (checkedInfo.checked) checkedInfo.checked = false;
                checkedInfo.disable = true;

                setTimeout(() => {
                    container_dataText.textContent = lessonDate;
                    container_topicText.textContent = lessonTopic;

                    checkedInfo.checked = true;
                    checkedInfo.disable = false;
                }, 1000);
            });
        }
    });
